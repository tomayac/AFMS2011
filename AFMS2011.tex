...
\begin{abstract}
%%%% Responsible: THOMAS %%%%
Many have left their footprints on the field of semantic RESTful Web service description. Albeit some of the propositions are even W3C Recommendation, none of the proposed standards could gain significant adoption with Web service providers. Some approaches were supposedly too complex and verbose, others were considered not RESTful, and some failed to reach a significant majority of API providers for a combination of the reasons above. While we neither have the silver bullet for universal Web service description, with this paper, we want to suggest a lightweight approach, which is based on pre- and postconditions expressed in simple N3 rules for the semantics of Web services, and that heavily reuses existing standards such as Link headers, HTTP OPTIONS, and URI templates for everything else. This approach keeps the complexity to a minimum while still allowing for full semantic expressiveness with regards to service description. We have deployed a sample implementation of our approach for testing purposes.
\end{abstract}
...
As a remark, we use the terms ``API" and ``(Web) service" synonymously.
...
It is also possible to generate WSDL or \owls\ descriptions, as all the information---besides human-targeted elements such as labels and textual information---are readily available. In fact, human-centric textual descriptions could also be generated based on RESTdesc and ontological information. \autoref{lst:PhotoGet} could translate to ``given an identifier, retrieves a representation of the photograph with that identifier.''
